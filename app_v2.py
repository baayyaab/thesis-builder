...
# 🛑 --- PAGE 6: กิตติกรรมประกาศ ---
    ...
    toc_items = [
     ...
     # ============================================
# เริ่ม Section ใหม่สำหรับเนื้อหาหลัก (Chapter 1)
# รีเซ็ตเลขหน้าใหม่
# ============================================

    chapter_section = doc.add_section()

    chapter_section.top_margin = Inches(1.5)
    chapter_section.bottom_margin = Inches(1.0)
    chapter_section.left_margin = Inches(1.5)
    chapter_section.right_margin = Inches(1.0)

# Header ของ Section นี้ไม่ใช้ของ Section ก่อนหน้า
    chapter_section.header.is_linked_to_previous = False

    header_para = chapter_section.header.paragraphs[0]
    header_para.alignment = WD_ALIGN_PARAGRAPH.RIGHT
    ...
    add_tu_style_page_number(page_run)

# เริ่มเลขหน้าใหม่ที่ 1
    sectPr = chapter_section._sectPr
    ...
    # ตั้งแต่บทที่ 2 เป็นต้นไป ขึ้นหน้าใหม่
      if i > 0:
        doc.add_page_break()
